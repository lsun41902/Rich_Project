...
class TickerManage:
    ...
    def init_ui(self):
        self.is_edit_mode = self.item_values is not None
        default_code = ""
        default_name = ""
        default_price = ""
        ...
        # 시장 선택 레이블 및 버튼
        tk.Label(self.add_win, text="시장 선택:", font=("Malgun Gothic", 9, "bold")).pack(pady=5)
        ...
        tk.Label(self.add_win, text="종목 코드 (숫자만):").pack(pady=5)
        ...
        # 실시간으로 표시될 라벨
        self.label_display = tk.Label(self.add_win, text="목표가: 0원", fg="blue")
    ...
    def save_new_ticker(self):
        ...
        if raw_code not in code_list:
            helper.show_message_box("경고", "코드를 확인해 주세요.", mtype=1)
