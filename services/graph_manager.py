...
class NewsGraphManager:
    ...
    def update_stock_data_smart(self, app, df, ticker_name, ticker_code, stock_type):
        import services.ui_helper as helper
        loading = helper.LoadingWindow(app.root)
        loading.show_progress("주가 정보를 가져오는 중 입니다.\n잠시만 기다려 주세요...")
        print(f"🆕 [최초 적재] {ticker_name}({ticker_code}) 데이터가 없습니다. 2년치 데이터를 로드합니다.")
        ...
        # 5. UNWIND를 사용한 일괄 적재 (Bulk Insert)
        batch_query = """
            UNWIND $batch AS row
            MERGE (p:StockPrice {id: row.node_id})
            SET p.date = date(row.date), 
